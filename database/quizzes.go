...
var GetQuizzes = func(filter string) ([]Quiz, error) {
	...
	for rows.Next() {
		var quiz Quiz
		if err = rows.Scan(&quiz.ID, &quiz.Name, &quiz.MaxScore, &quiz.ImageURL, &quiz.Verb, &quiz.APIPath, &quiz.HasLeaderboard, &quiz.Enabled); err != nil {
			return nil, err
		}
...
var GetQuiz = func(id int) (Quiz, error) {
	...
	var quiz Quiz
	err := Connection.QueryRow(statement, id).Scan(&quiz.ID, &quiz.Name, &quiz.MaxScore, &quiz.ImageURL, &quiz.Verb, &quiz.APIPath, &quiz.HasLeaderboard, &quiz.Enabled)
	return quiz, err
}
