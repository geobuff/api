...
var GetLeaderboardEntry = func(table string, userID int) (LeaderboardEntryDto, error) {
	statement := fmt.Sprintf("SELECT * from (SELECT l.id, userid, u.username, q.id, q.name, l.countrycode, score, time, added, RANK () OVER (ORDER BY score desc, time) rank FROM %s l JOIN users u on u.id = l.userId JOIN quizzes q on q.id = $1) c WHERE c.userid = $2;", table)
	var entry LeaderboardEntryDto
